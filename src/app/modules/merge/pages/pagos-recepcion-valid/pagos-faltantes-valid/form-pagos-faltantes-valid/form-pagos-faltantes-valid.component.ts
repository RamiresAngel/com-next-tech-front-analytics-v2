...
@Component({
  selector: 'app-form-pagos-faltantes-valid',
  templateUrl: './form-pagos-faltantes-valid.component.html',
  styleUrls: ['./form-pagos-faltantes-valid.component.scss']
})
export class FormPagosFaltantesValidComponent {
  ...
  iniciaFormFiltro(): void {
    this.formFilters = new FormGroup({
      ...
      cpReceptorControl: new FormControl(''),
  ...
  public obtenerRfcEmisores(event: any): any {
    ...
    this._mergeService.getRFCMap({
      ...
      this.formFilters.controls['rfcEmisorControl'].patchValue(this.rfcEmisores[0].rfc);
    });
  };
  ...
  public enviarDatosFiltro(): void {
    this.body_merge.filtro = this._mergeUtils.getBodyFiltroMerge(this.formFilters.value);

    this.body_merge.tipo = 'recepcion_valid';
    this.body_merge.tipo_comprobante = 'pagos';
  ...
  onChange(event: any) {
    ...
    this.formFilters.patchValue({ fechaFacturaControl: fecha });
  }
